...
module.exports = (db) => {
  ...
  transporter.verify((error, success) => {
    if (error) {
      console.error("[v0] Email transporter verification failed:", error)
    } else {
  ...
  router.post("/forgot-password", async (req, res) => {
    ...
    try {
      ...
      if (!user) {
        ...
        console.log("[FORGOT-PASSWORD] Returning generic success message (security)")
        return res.json({ message: "If the email exists, a password reset link has been sent." })
      }

      console.log("[FORGOT-PASSWORD] Step 4: User FOUND in database")
      console.log("[FORGOT-PASSWORD] User email:", user.email)
      console.log("[FORGOT-PASSWORD] User verified:", user.verified)

      console.log("[FORGOT-PASSWORD] Step 5: Generating reset token...")
      const resetToken = generateResetToken()
      const resetTokenExpiry = new Date(Date.now() + 60 * 60 * 1000) // 1 hour from now
      console.log("[FORGOT-PASSWORD] Reset token generated (first 10 chars):", resetToken.substring(0, 10) + "...")
      console.log("[FORGOT-PASSWORD] Token expiry:", resetTokenExpiry)

      console.log("[FORGOT-PASSWORD] Step 6: Saving reset token to database...")
      ...
      console.log("[FORGOT-PASSWORD] Step 7: Frontend URL:", frontendUrl)

      const resetUrl = `${frontendUrl}/reset-password/${resetToken}`
      console.log("[FORGOT-PASSWORD] Step 8: Reset URL generated:", resetUrl)

      // Check if email credentials are configured
      console.log("[FORGOT-PASSWORD] Step 9: Checking email configuration...")
      ...
      console.log("[FORGOT-PASSWORD] Step 10: Preparing to send email...")
      ...
      try {
        ...
        console.error("[FORGOT-PASSWORD] Full error object:", JSON.stringify(emailError, null, 2))
